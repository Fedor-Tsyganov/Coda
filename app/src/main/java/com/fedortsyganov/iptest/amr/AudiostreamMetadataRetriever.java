...
public class AudiostreamMetadataRetriever implements Runnable
{
    ...
    public void run()
    {
        ...
        headers_data.putStringArrayList(IcecastHeader.NAME, headers.get(IcecastHeader.NAME) != null ? new ArrayList<String>(headers.get(IcecastHeader.NAME)) : new ArrayList<String>());
        headers_data.putStringArrayList(IcecastHeader.DESC, headers.get(IcecastHeader.DESC) != null ? new ArrayList<String>(headers.get(IcecastHeader.DESC)) : new ArrayList<String>());
        headers_data.putStringArrayList(IcecastHeader.BR, headers.get(IcecastHeader.BR) != null ? new ArrayList<String>(headers.get(IcecastHeader.BR)) : new ArrayList<String>());
        headers_data.putStringArrayList(IcecastHeader.GENRE, headers.get(IcecastHeader.GENRE) != null ? new ArrayList<String>(headers.get(IcecastHeader.GENRE)) : new ArrayList<String>());
        headers_data.putStringArrayList(IcecastHeader.INFO, headers.get(IcecastHeader.INFO) != null ? new ArrayList<String>(headers.get(IcecastHeader.INFO)) : new ArrayList<String>());
        ...
        try
        {
            while (!Thread.interrupted())
            {
                ...
                if (metaDataLength > 0)
                {
                    ...
                    String result = baos.toString().replace("StreamTitle=", "").replaceAll("'", "").replaceAll(";", "").replaceAll("http.*","").replace("StreamUrl=", "");
                    baos.reset();
